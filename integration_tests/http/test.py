...
def test_basic_http():
    ...
    run_command(f"CONTAINER_NAME={container_name} {zgrab_root}/docker-runner/docker-run.sh http",
                output_file=os.path.join(output_root, "http.json"))

def test_basic_https():
    print("http/test: Run https test on port 443")
    run_command(f"CONTAINER_NAME={container_name} {zgrab_root}/docker-runner/docker-run.sh http --port 443 --use-https",
...
def test_http_body_contents():
    ...
    if expected_content != actual_content:
        ...
        print(diff)
        raise ValueError("http/body-test: The HTTP body contents do not match the expected contents.\n")
    else:
        pass
...
def test_large_http_body_contents():
    with open("./container/index-very-large-http.html", "r", encoding="utf-8") as f:
        expected_content = f.read()
    # remove the trailing newlines that jq is adding and extract body from ZGrab scan
    cmd = f"CONTAINER_NAME={container_name} {zgrab_root}/docker-runner/docker-run.sh http --endpoint=/large.html | jq -r '.data.http.result.response.body' | perl -pe 'chomp if eof'"
    actual_content = run_command(cmd)
    if expected_content != actual_content:
        diff = difflib.unified_diff(expected_content.splitlines(), actual_content.splitlines(), lineterm="")
        line_ctr = 0
        for line in diff:
            ...
            if line_ctr > 10:
                ...
                break
        raise ValueError("http/large-body-test: The HTTP body contents do not match the expected contents.\n")
    else:
        pass
