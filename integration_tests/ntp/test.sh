...
function test_openntp() {
    ...
    CONTAINER_NAME="zgrab_ntp_openntp" $ZGRAB_ROOT/docker-runner/docker-run.sh ntp --monlist > out.tmp
...
function test_bad_req() {
    code=$1
    expected_error=$2
    CONTAINER_NAME="zgrab_ntp_4.2.6" $ZGRAB_ROOT/docker-runner/docker-run.sh ntp --monlist --request-code $code --skip-get-time > out.tmp
...
function test_4_2_6() {
    ...
    for code in $request_codes; do
        ...
        CONTAINER_NAME=$CONTAINER_NAME $ZGRAB_ROOT/docker-runner/docker-run.sh ntp --monlist --request-code $code --skip-get-time > "$OUTPUT_ROOT/4.2.6_$code_solo.json"
    done
