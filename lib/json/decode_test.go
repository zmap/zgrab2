...
func TestUnmarshal(t *testing.T) {
	for i, tt := range unmarshalTests {
		...
		if !reflect.DeepEqual(v.Elem().Interface(), tt.out) {
			...
			data, _ := Marshal(v.Elem().Interface(), false)
			println(string(data))
			data, _ = Marshal(tt.out, false)
			println(string(data))
			continue
		}
		...
		if tt.err == nil {
			enc, err := Marshal(v.Interface(), false)
			if err != nil {
