...
func RegisterModule() {
	var module Module
	_, err := zgrab2.AddCommand("http_proxy", "http_proxy", "check http_proxy", 3128, &module)
	if err != nil {
		log.Fatal(err)
	}
}
...
func (s *Scanner) Scan(t zgrab2.ScanTarget) (status zgrab2.ScanStatus, result interface{}, thrown error) {
	var err error
	raw_url := fmt.Sprintf("http://%s:%d", t.IP.String(), s.GetPort())
	proxyUrl, err := url.Parse(raw_url)
	...
	if err != nil {
		return zgrab2.SCAN_IO_TIMEOUT, ProxyResult{OK: "failed"}, err
	} else {
		if resp.StatusCode == http.StatusOK {
			body, err := ioutil.ReadAll(resp.Body)
			if err!=nil{
				return zgrab2.SCAN_UNKNOWN_ERROR, ProxyResult{"failed"}, err
			} else if strings.Contains(string(body), "illustrative examples") {
				return zgrab2.SCAN_SUCCESS, ProxyResult{OK: raw_url}, nil
			} else {
				return zgrab2.SCAN_PROTOCOL_ERROR, ProxyResult{OK: "failed"}, errors.New("Proxy Error")
			}
		}
		return zgrab2.SCAN_PROTOCOL_ERROR, ProxyResult{OK:"failed"},errors.New("Response not 200")
	}
}
