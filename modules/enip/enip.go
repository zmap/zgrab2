...
const (
	...
	EnipUnregisterSession = 0x0066
	EnipSendPRData        = 0x006F
	EnipSendUintData      = 0x0070
	EnipStartDtls         = 0x00C8
)

// EnipStatusCode
const (
	EnipStatusSuccess            = 0x0000
	EnipStatusInvalidCmd         = 0x001
	EnipStatusNoResources        = 0x0002
	EnipStatusIncorrectData      = 0x0003
	EnipStatusInvalidSession     = 0x0064
	EnipStatusInvalidLength      = 0x0065
	EnipStatusUnsupportedProtRev = 0x0069
	EnipStatusENcapHeaderError   = 0x006A
)

var EnipStatusMap = map[int]string{
	EnipStatusSuccess:            "Sucess",
	EnipStatusInvalidCmd:         "Invalid Cmd",
	EnipStatusNoResources:        "No Resources",
	EnipStatusIncorrectData:      "Incorrect Data",
	EnipStatusInvalidSession:     "Invalid Session",
	EnipStatusInvalidLength:      "Invalid Length",
	EnipStatusUnsupportedProtRev: "Unsupported Prot Rev",
	EnipStatusENcapHeaderError:   "ENcap Header Error",
}

var EnipCommandMap = map[int]string{
	EnipNOP:               "NOP",
	EnipListServices:      "List Services",
	EnipListIdentity:      "List Identity",
	EnipListInterfaces:    "List Interfaces",
	EnipRegisterSession:   "Register Session",
	EnipUnregisterSession: "Unregister Session",
	EnipSendPRData:        "Send PR Data",
	EnipSendUintData:      "Send Uint Data",
	EnipStartDtls:         "Start Dtls",
}

var EnipVendorMap = map[int]string{
	0:    "Reserved",
...
var EnipDeviceTypeMap = map[int]string{
...
func (header *EnipHeader) IsError() bool {
	return header.Status != EnipStatusSuccess
}

func (header *EnipHeader) GetStatusString() (string, error) {
	val, ok := EnipStatusMap[int(header.Status)]
	if !ok {
		return val, errors.New("unknown status")
	}

	return val, nil
}

func (header *EnipHeader) GetCommandString() (string, error) {
	val, ok := EnipCommandMap[int(header.Command)]
	if !ok {
		return val, errors.New("unkown command")
	}

	return val, nil
}

func (header *EnipHeader) Marshal() []byte {
	headerBytes := make([]byte, EnipHeaderSize)
	binary.LittleEndian.PutUint16(headerBytes[0:2], header.Command)
	binary.LittleEndian.PutUint16(headerBytes[2:4], header.Length)
	binary.LittleEndian.PutUint32(headerBytes[4:8], header.SessionHandle)
	binary.LittleEndian.PutUint32(headerBytes[8:12], header.Status)
	binary.LittleEndian.PutUint64(headerBytes[12:20], header.SenderContext)
	binary.LittleEndian.PutUint32(headerBytes[20:24], header.Options)

	return headerBytes
}

func (header *EnipHeader) UnMarshal(headerBytes []byte) error {
	if len(headerBytes) >= EnipHeaderSize {
		header.Command = binary.LittleEndian.Uint16(headerBytes[0:2])
		header.Length = binary.LittleEndian.Uint16(headerBytes[2:4])
		header.SessionHandle = binary.LittleEndian.Uint32(headerBytes[4:8])
		header.Status = binary.LittleEndian.Uint32(headerBytes[8:12])
		header.SenderContext = binary.LittleEndian.Uint64(headerBytes[12:20])
		header.Options = binary.LittleEndian.Uint32(headerBytes[20:24])
		return nil
	}
	return errors.New("not enough data in the packet")

}

const CIPIdentityType = 0x000c
...
const CIPIdentityMinSize = 33

func (identity *CIPIdentity) GetVendorString() (string, error) {
	val, ok := EnipVendorMap[int(identity.VendorID)]
	if !ok {
		return val, errors.New("unknown vendor")
	}

	return val, nil
}
func (identity *CIPIdentity) GetDeviceTypeString() (string, error) {
	val, ok := EnipDeviceTypeMap[int(identity.DeviceType)]
	if !ok {
		return val, errors.New("unknown device type")
	}

	return val, nil
}

func (identity *CIPIdentity) UnMarshal(identityBytes []byte) error {
	if len(identityBytes) >= CIPIdentityMinSize {
		identity.ProtocolVersion = binary.LittleEndian.Uint16(identityBytes[0:2])
		identity.SocketAddress.SinFamily = binary.LittleEndian.Uint16(identityBytes[2:4])
		identity.SocketAddress.SinPort = binary.LittleEndian.Uint16(identityBytes[4:6])
		identity.SocketAddress.SinAddr = binary.LittleEndian.Uint32(identityBytes[6:10])
		identity.SocketAddress.SinZero = binary.LittleEndian.Uint64(identityBytes[10:18])
		identity.VendorID = binary.LittleEndian.Uint16(identityBytes[18:20])
		identity.DeviceType = binary.LittleEndian.Uint16(identityBytes[20:22])
		identity.ProductCode = binary.LittleEndian.Uint16(identityBytes[22:24])
		identity.Revision = binary.LittleEndian.Uint16(identityBytes[24:26])
		identity.Status = binary.LittleEndian.Uint16(identityBytes[26:28])
		identity.Serial = binary.LittleEndian.Uint32(identityBytes[28:32])
...
func (c *EnipCon) Send(command int, payload []byte) (int, error) {
	header := EnipHeader{Command: uint16(command), Length: uint16(len(payload)), SessionHandle: c.Session,
		Status: EnipStatusSuccess, SenderContext: 0, Options: 0}
...
func (c *EnipCon) Recv() (EnipHeader, []byte, error) {
	buffer := make([]byte, EnipHeaderSize, 4096)
	header := EnipHeader{}
	read, err := c.Conn.Read(buffer)
	if err != nil {
		return header, nil, err
	} else if read != EnipHeaderSize {
		return header, nil, errors.New("failed to read enip header")
	}
	err = header.UnMarshal(buffer)
	if err != nil {
		return EnipHeader{}, nil, err
	} else if int(header.Length) > cap(buffer) {
		return header, nil, errors.New("enip payload too big to handle")
	}
	buffer = buffer[:header.Length]
	read, err = c.Conn.Read(buffer)
	if err != nil {
		return header, nil, err
	} else if read != int(header.Length) {
		return header, nil, errors.New("failed to read enip payload")
	}

	return header, buffer, nil
}

func (c *EnipCon) GetCIPIdentity() (CIPIdentity, error) {
	// Send the request
	identity := CIPIdentity{}
	_, err := c.Send(EnipListIdentity, []byte{})
	if err != nil {
		return identity, err
	}
	...
	if header.Command != EnipListIdentity {
		return identity, fmt.Errorf("got response from enip device with unexpcted command: %d",
			header.Command)
	} else if header.IsError() {
		return identity, fmt.Errorf("got response from enip device with unexpcted status: %d",
			header.Status)
	} else if len(payload) > 4 {
		// Check that it received 1 item at least
		itemCount := binary.LittleEndian.Uint16(payload[0:2])
		offset := 2
		...
		return identity, fmt.Errorf("got response from enip device with not enough data: %d",
			len(payload))
	}

	return identity, errors.New("failed to find CIP identity in the response")
}
